...
'''
测试接口：登录	GET	/v2/book/:id
测试URL：https://devtest5.buildingqm.com//uc/user/login/?_ct=json
测试数据: hrsz + huarun123
预期结果：
...
def test_get_login():
    user_name = 'hrsz'
    password = 'huarun123'
    response = requests.get("https://devtest5.buildingqm.com//uc/user/login/?_ct=json" + user_name + password, verify=True)
    print(response.url)
